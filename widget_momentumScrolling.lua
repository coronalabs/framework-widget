		...
		if "moved" == phase then
			...
			if "horizontal" == view._moveDirection then
				...
				if not view._isHorizontalScrollingDisabled then
					...
					print( not view._isHorizontalScrollingDisabled )
					
					view._delta = event.x - view._prevXPos
					view._prevXPos = event.x
				
					-- Limit movement
					local leftLimit = view._background.x - ( view.contentWidth ) + ( view._background.contentWidth * 0.5 )
					local rightLimit = 0
			...
			-- Vertical movement
				...
				if not view._isVerticalScrollingDisabled then
					...
					-- Limit movement
					local upperLimit = view._background.y - ( view.contentHeight ) + ( view._background.contentHeight * 0.5 ) - view._bottomPadding
	...
	if view._updateRuntime then
		...
		if "horizontal" == view._moveDirection then
			...
			if not view._isHorizontalScrollingDisabled then
				...
				local leftLimit = view._background.x - ( view.contentWidth ) + ( view._background.contentWidth * 0.5 ) - view._rightPadding
